...
public class BreedingCategory extends AbstractRecipeCategory<BreedingRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, BreedingRecipe recipe, @NotNull IFocusGroup focuses) {
        ...
        if (hasOutput) {
            ...
            builder.addOutputSlot(130, 48).setOutputSlotBackground().add(recipe.outputs()).setPosition(72, 38, 78, 35, HorizontalAlignment.RIGHT, VerticalAlignment.CENTER);
            if (hasExtraInput) {
                inputSlot.setPosition(74, 38, 78, 35, HorizontalAlignment.LEFT, VerticalAlignment.BOTTOM);
                builder.addInputSlot(69, 33).setStandardSlotBackground().add(recipe.extraInputs()).setPosition(74, 38, 78, 35, HorizontalAlignment.LEFT, VerticalAlignment.TOP);
            }
