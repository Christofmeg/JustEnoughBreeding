...
public class TransformationSerializer implements RecipeSerializer<TransformationRecipe> {
    ...
    public static BaseRecipe readJsonContents(@NotNull ResourceLocation jsonPath, @NotNull JsonObject json) {
        ...
        if (inputSpawnEggs.isEmpty()) {
            ItemStack spawnEgg = Optional.ofNullable(JustEnoughBreeding.getSpawnEggItem(inputEntityType)).map(SpawnEggItem::getDefaultInstance).orElse(ItemStack.EMPTY);
            inputSpawnEggs = new ArrayList<>(List.of(Ingredient.of(spawnEgg)));
        }
        if (outputSpawnEggs.isEmpty()) {
            ItemStack spawnEgg = Optional.ofNullable(JustEnoughBreeding.getSpawnEggItem(outputEntityType)).map(SpawnEggItem::getDefaultInstance).orElse(ItemStack.EMPTY);
